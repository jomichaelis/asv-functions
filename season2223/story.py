...
def create_image(home, guest, team_home, team_guest, rank_home, rank_guest, date, time, liga):
	...
	font_teams = ImageFont.truetype(os.path.join(fonts_path, 'BebasNeue.ttf'), 105)
	...
	draw.text((540-(home_team_width1/2), 1100), home_team_name, (0, 0, 0), font=font_teams)
	draw.text((540-(minus_width1/2), 1200), minus, (0, 0, 0), font=font_teams)
	draw.text((540-(guest_team_width1/2), 1300), guest_team_name, (0, 0, 0), font=font_teams)
	# TODO: Fix width of text if too long

	# paste date
	# print(date, time)
	font_date = ImageFont.truetype(os.path.join(fonts_path, 'Manrope-Regular.ttf'), 62)
	date_a_width, nw = draw.textsize(date, font=font_date)
	date_b_width, nw = draw.textsize(time, font=font_date)
	draw.text(((1080-date_a_width)/2, 1600), date, (255, 255, 255), font=font_date)
	draw.text(((1080-date_b_width)/2, 1700), time, (255, 255, 255), font=font_date)

	# paste liga
	font_liga = ImageFont.truetype(os.path.join(fonts_path, 'BebasNeue.ttf'), 70)
	liga_width, nw = draw.textsize(liga, font=font_liga)
	draw.text(((1080-liga_width)/2+3, 152), liga, (190, 176, 0), font=font_liga)
	draw.text(((1080-liga_width)/2, 150), liga, (0, 0, 0), font=font_liga)
